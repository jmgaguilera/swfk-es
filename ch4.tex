...
En términos de programación, una pregunta normalmente significa que queremos hacer o una cosa u otra, dependiendo de la respuesta a la pregunta.  A la sentencia que permite esto de la denomina \textbf{sentencia if}\index{if-statement}\footnote{En inglés `if' significa `si'}.  Por ejemplo:
...
\par
Una condición\index{conditions} es un cálculo de programación cuyo resultado es `Sí' (True) o `No' (False).  Hay ciertos símbolos (u operadores) que se utilizan para crear condiciones, como son:
...
\section{Combinando condiciones}\index{conditions!combining}
...
\section{¿Cuál es la diferencia$\ldots$?}\label{whatsthedifference}\index{equality}
